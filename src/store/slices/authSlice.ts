...
export interface User {
  ...
  email: string;
  name: string;
  role?: string; // 'super-admin', 'admin', 'employee', etc.
  storeId?: string; // For employees
  employeeId?: string; // For employees
  permissions?: any[]; // Employee permissions
}
...
export const loginUser = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      console.log("AuthSlice - Login response data:", response.data);
      return response.data;
    } catch (error: any) {
      console.error("AuthSlice - Login error:", error);
      console.error("AuthSlice - Login error response:", error.response);
      const errorMessage = error.response?.data?.message || "Login failed";
      console.error("AuthSlice - Returning error:", errorMessage);
