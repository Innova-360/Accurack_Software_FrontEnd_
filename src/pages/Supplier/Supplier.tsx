...
const SupplierPage: React.FC = () => {
  ...
  const { currentStore, isStoreReady } = useStoreFromUrl();
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (isStoreReady && currentStore?.id) {
        try {
    ...
    fetchData();
  }, [dispatch, currentStore?.id, isStoreReady]);
  ...
  // Show loading while store is being determined
  if (!isStoreReady) {
    return (
      <>
        <Header />
