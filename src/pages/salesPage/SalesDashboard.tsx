...
const SalesDashboard: React.FC = () => {
  ...
  useEffect(() => {
    console.log("🔍 Sales Page Debug Info:", {
      ...
      totalItemsCalculated:
        isServerSidePagination && pagination?.total !== undefined
          ? pagination.total
  ...
  const handleUploadSalesModal = async () => {
    setIsSaleCreationModalOpen(false);
    setIsUploadSalesModalOpen(true);
  };
  ...
  return (
    <>
      ...
      <SaleCreationModal
        ...
        storeId={storeId}
        handleUploadSalesModal={handleUploadSalesModal}
      />
      <UploadSalesModal
        isOpen={isUploadSalesModalOpen}
        onClose={() => setIsUploadSalesModalOpen(false)}
        onUploadSuccess={fetchSalesData}
      />
    </>
  );
};
