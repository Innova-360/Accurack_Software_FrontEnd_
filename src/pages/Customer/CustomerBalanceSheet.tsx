...
const CustomerBalanceSheet: React.FC = () => {
  ...
  const { id: storeId, customerId } = useParams<{
    id: string;
    customerId: string;
  }>();
  ...
  const filteredPayments = (balanceData?.balanceHistory || []).filter(
    (payment: BalanceHistoryItem) => {
      const matchesSearch =
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600">
              Please return to the customer list and try again.
            </p>
            <SpecialButton
              onClick={handleBack}
              variant="primary"
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {balanceData.balanceHistory.length}
                </p>
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredPayments
                  .filter(
                    (payment) =>
                      payment.description !== "Initial balance sheet created"
                  )
                  .map((payment, index) => (
                    <tr
                      key={payment.id}
                    ...
                    >
                      ...
                      <td className="px-1 py-3 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-200">
                        <span className=" font-mono font-semibold">
                          $
                          {payment.remainingAmount.toLocaleString("en-US", {
                            minimumFractionDigits: 0,
                            maximumFractionDigits: 0,
                          })}
                        </span>
                ...
                <tr className="border-t-2 border-gray-900 bg-gray-100">
                  <td
                    className="px-6 py-4 text-sm font-bold text-gray-900 border-r border-gray-300"
                    colSpan={3}
                  >
                    Total
                  </td>
                  <td className="px-6 py-4 text-left text-sm font-bold border-r border-gray-300">
                    <span className="font-mono ">
                      $
                      {balanceData.totalPaid.toLocaleString("en-US", {
                        minimumFractionDigits: 0,
                        maximumFractionDigits: 0,
                      })}
                    </span>
                  ...
                  >
                    <span className="font-mono ">
                      Total Balance: $
                      {(
                        balanceData.currentBalance + balanceData.totalPaid
