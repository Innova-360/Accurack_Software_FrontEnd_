...
import ToastConfirmation, {
  showConfirmationToast,
} from "../reusablecomponents/ToastConfirmation";
...
const InventoryTable: React.FC<InventoryTableProps> = ({
  ...
  const handleVariantDelete = (variant: any, productName: string) => {
    const variantDisplayName =
      variant.name || `Variant ${variant.id?.slice(0, 8) || "Unknown"}`;
    ...
    showConfirmationToast(
      ...
      async () => {
        try {
          ...
          // Show success toast
          toast.success(
            `Variant "${variantDisplayName}" deleted successfully!`
          );
  ...
  const handleVariantBulkDelete = async (selectedPluUpcs: string[]) => {
    ...
    try {
      ...
      // Show success toast
      toast.success(
        `Successfully deleted ${selectedPluUpcs.length} variant${selectedPluUpcs.length !== 1 ? "s" : ""}!`
      );
            ...
            <tbody>
              {products.length > 0 ? (
                products.map((product, index) => {
                  ...
                  const baseVariant =
                    hasVariantsToShow && product.variants?.[0]
                      ? product.variants[0]
                      : null;
                              ...
                              <div className="font-medium truncate">
                                {hasVariantsToShow && baseVariant
                                  ? typeof baseVariant.name === "string"
                                    ? baseVariant.name === "Default Variant"
                                      ? product.name
                                      : baseVariant.name
                            ...
                            editingQuantity.variantIndex ===
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  ...
                                  title="Save quantity"
                                  disabled={updatingQuantity.has(
                                    hasVariantsToShow
                                      ? `${productKey}-variant-0`
                                      : productKey
                                  )}
                                >
