...
const PERMISSIONS: Permission[] = [
  { id: 'create', name: 'Create', icon: <FaPlus className="w-4 h-4 text-green-600" /> },
  { id: 'read', name: 'Read', icon: <FaEye className="w-4 h-4 text-blue-600" /> },
  { id: 'update', name: 'Update', icon: <FaEdit className="w-4 h-4 text-orange-600" /> },
  { id: 'delete', name: 'Delete', icon: <FaTrash className="w-4 h-4 text-red-600" /> },
  { id: 'export', name: 'Export', icon: <FaChartLine className="w-4 h-4 text-purple-600" /> },
  { id: 'manage', name: 'Manage', icon: <FaUserShield className="w-4 h-4 text-indigo-600" /> },
];
...
const Permissions: React.FC<PermissionsProps> = ({ showHeader = true, onEditEmployee }) => {
...
//   const [storeFilter, setStoreFilter] = useState('All Stores');
  ...
  const convertRoleToEmployeeData = (role: Role) => {
    ...
    const permissionsObject = role.permissions.reduce((acc, permission) => {
            ...
            <table className="w-full">              <thead className="bg-gray-50 border-b border-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {paginatedRoles.map((role) => (
                  <tr key={role.id} className="hover:bg-gray-50">                    {/* Role Name */}
                    ...
                    {/* Employee Name */}
                    <td className="py-4 px-4">
                      <span className="text-gray-600 text-sm">{role.employeeName}</span>
                    </td>

                    {/* Employee ID */}
                    <td className="py-4 px-4 text-center">
                      <span className="text-gray-600 text-sm font-medium">{role.employeeId}</span>
                    </td>

                    {/* Created Date */}
                    <td className="py-4 px-4 text-center">
                      <span className="text-gray-600 text-sm">{role.createdDate}</span>
                    </td>
