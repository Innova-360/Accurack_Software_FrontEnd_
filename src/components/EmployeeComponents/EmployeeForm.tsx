  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && formData.id) {
        ...
        console.log("Password being sent:", formData.password || 'TempPassword123!');
        
        // Creating new employee
        const result = await dispatch(createEmployee(submitData)).unwrap();
        console.log("Employee creation result:", result);
        console.log("Employee ID from result:", result.id || result.data?.id || result.employee?.id);
        
        // Send email invite after employee creation with login credentials
        try {
          const employeeId = result.id || result.data?.id || result.employee?.id;
          const invitePayload = {
          ...
          try {
            ...
            const verifyResponse = await apiClient.get(`/employees/${employeeId}`);
            console.log("Employee verification response:", verifyResponse.data);
            
            if (verifyResponse.data && verifyResponse.data.status === 'active') {
              console.log("Employee is active and ready for login");
            } else {
              console.warn("Employee created but may not be active:", verifyResponse.data);
            }
          } catch (verifyError) {
            console.error("Failed to verify employee creation:", verifyError);
          }
          
          toast.success('Employee created and invitation email sent successfully!');
        } catch (inviteError) {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-white font-medium">Personal Info</span>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-white font-medium">Role & Permissions</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="mb-10">
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-xl font-bold text-[#043E49]">Personal Information</h3>
              </div>
            ...
            <div className="mb-10">
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-xl font-bold text-[#043E49]">Employee Information</h3>
              </div>
                ...
                <div className="md:col-span-2 lg:col-span-1 space-y-2">
                  ...
                  {!showCreateRole ? (
                    ...
                    >
                      ...
                      {roleTemplatesLoading && <option>Loading roles...</option>}
                      {roleTemplates &&
