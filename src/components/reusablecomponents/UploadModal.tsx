...
interface UploadModalProps {
  ...
  title: string;
  templateData: Array<Array<string>>;
...
const UploadModal: React.FC<UploadModalProps> = ({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      if (isValidFileType(file)) {
        ...
        toast.error(`Please upload only Excel files (${acceptedFileTypes})`);
      }
    }
  };
  ...
  const handleUpload = async () => {
    ...
    const url = `${BASE_URL}${uploadEndpoint}?storeId=${id}`;
    ...
    try {
      ...
      toast.success(`${title.split(" ")[1]} uploaded successfully!`);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl p-6 m-4 w-full max-w-lg animate-modal-enter">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-[#0f4d57]">{title}</h2>
          <button
            onClick={handleClose}
